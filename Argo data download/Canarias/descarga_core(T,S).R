...
# Subset by space.
rect <- list(longitude=c(-25,-5), latitude=c(15,45))
...
library(dplyr)
library(data.table)
setnames(df,colnames(df1))

nombres<-c("Fecha","Lon", "Lat","Pres","Pres_Adj","Temp","Temp_Adj","Sal","Sal_Adj","Ciclo","Id")
colnames(df)<-nombres

df1<-df%>%filter(Pres_Adj< 601)

df1<-df1%>%rename(Fecha=time_x, Lon=Lon_x, Lat=Lat_x,Ciclo=Cyc_x, Id= Id_x )
# Guardar el dataset final
ruta="C:/Users/Windows/Desktop/Argo Tesis/Canarias/Core/"
...
setwd(ruta)
##################################################################################
df <- list.files(path="C:/Users/Windows/Desktop/Argo Tesis/Canarias/Core") %>% 
  lapply(read_csv) %>% 
  bind_rows 
